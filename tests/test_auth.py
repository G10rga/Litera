...
def test_retired_study_paths_redirect(client):
    response = client.get("/syllabus", follow_redirects=False)
    assert response.status_code == 301
    assert "/literature" in response.headers["Location"]

    response = client.get("/moderntraslations", follow_redirects=False)
    assert response.status_code == 301
    assert "vefxistyaosani" in response.headers["Location"]
